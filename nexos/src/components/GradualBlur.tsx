...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = props => {
  ...
  useEffect(() => {
    if (!config.hideAtSelector) return;
    const footerEl = document.querySelector(config.hideAtSelector);
    if (!footerEl) return;
    ...
    return () => observer.disconnect();
  }, [config.hideAtSelector]);

  const effectiveOpacity = footerVisible ? 0 : (isVisible ? 1 : 0);
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const rh = responsiveHeight as string | undefined;
    const rw = responsiveWidth as string | undefined;

    if (isVertical) {
      baseStyle.height = rh;
      baseStyle.width = rw || '100%';
      baseStyle[config.position] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
    } else if (isHorizontal) {
      baseStyle.width = rw || rh;
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, effectiveOpacity]);

  const { hoverIntensity, animated, onAnimationComplete, duration } = config;
  useEffect(() => {
    if (isVisible && animated === 'scroll' && onAnimationComplete) {
      const t = setTimeout(() => onAnimationComplete(), parseFloat(duration as string) * 1000);
