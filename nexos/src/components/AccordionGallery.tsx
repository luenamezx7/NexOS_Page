...
const AccordionGallery = ({
  ...
  const applyLayout = useCallback((animate: boolean) => {
    ...
    panels.forEach((panel, i) => {
      ...
      tl.to(panel, { flexGrow: isActive ? grow : 1, ...rotProp, duration: dur, ease } as never, 0);
      if (media) {
        const drift = Math.max(-1.5, Math.min(1.5, active - i));
        const shift = drift * parallax * mediaSize * 0.06;
        const gray = grayscale ? (isActive ? 0 : 1) : 0;
        tl.to(media, { xPercent: -50, yPercent: -50, x: vertical ? 0 : isActive ? 0 : shift, y: vertical ? (isActive ? 0 : shift) : 0, '--ag-gray': gray, '--ag-dim': isActive ? 0 : 0.35, duration: dur, ease } as never, 0);
      }
      if (showLabels && bar && text) {
        if (isActive) tl.to([bar, text], { opacity: 1, x: 0, duration: dur, ease, stagger: prefersReduced ? 0 : stagger } as never, 0);
        else tl.to([bar, text], { opacity: 0, x: -14, duration: dur * 0.6, ease } as never, 0);
      }
    });
    tlRef.current = tl;
  ...
  return (
    <div ref={rootRef} className={`accordion-gallery${vertical ? ' accordion-gallery--vertical' : ''}${className ? ` ${className}` : ''}`} style={rootStyle} role="list" aria-label="Image accordion gallery">
      {items.map((item, i) => {
        const isActive = i === active;
        ...
        return (
          <Tag
            ...
            onClick={(e) => handleClick(i, e as never)}
          ...
          >
            <span className="ag-panel__frame">
              <span className="ag-panel__media" ref={(el: HTMLElement | null) => { mediaRefs.current[i] = el; }}>
                <img src={item.image} alt={item.alt || item.label || ''} draggable={false} width="900" height="1200" />
              </span>
