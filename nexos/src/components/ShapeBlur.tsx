...
float strokeAA(float x, float size, float w, float edge) {
    ...
    float d = smoothstep(size - edge - afwidth, size + edge + afwidth, x + w * 0.5) - smoothstep(size - edge - afwidth, size + edge + afwidth, x - w * 0.5);
    return clamp(d, 0.0, 1.0);
}
void main() {
    vec2 st = st0 + 0.5;
...
const ShapeBlur = ({
  ...
  useEffect(() => {
    ...
    document.addEventListener('pointermove', onPointerMove as never);
    const resize = () => {
      if (!active) return;
      w = mount.clientWidth;
    ...
    const ro = new ResizeObserver(() => { if (!active) return; resize(); });
    ro.observe(mount);
    const update = () => {
      if (!active) return;
    ...
    return () => {
      ...
      document.removeEventListener('pointermove', onPointerMove as never);
      if (mount.contains(renderer.domElement)) mount.removeChild(renderer.domElement);
      geo.dispose(); material.dispose(); materialRef.current = null; renderer.dispose(); renderer.forceContextLoss();
    };
  }, [variation]);
