...
const Topography = ({
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      (u.iTime as { value: number }).value = time;
      const ma = (u.uMorphAmount as { value: number }).value;
      const sp = (u.uSpeed as { value: number }).value;
      const msp = (u.uMorphSpeed as { value: number }).value;
      for (let g = 0; g < 4; g++) {
      ...
      (u.uMouse as { value: Float32Array }).value[0] = currentMouse[0];
      (u.uMouse as { value: Float32Array }).value[1] = currentMouse[1];
      mouseActive += 0.05 * (mouseActiveTarget - mouseActive);
      (u.uMouseActive as { value: number }).value = mouseActive;
      renderer.render({ scene: mesh });
    ...
    let rafId = 0;
    const tryStart = () => {
      if (isVisible && isPageVisible && rafId === 0) rafId = requestAnimationFrame(loop);
    };
    const tryStop = () => {
      if (rafId !== 0) {
  ...
  useEffect(() => {
    ...
    (u.uMorphAmount as { value: number }).value = morphAmount;
    (u.uMorphSpeed as { value: number }).value = morphSpeed;
    (u.uBands as { value: number }).value = bands;
    (u.uThickness as { value: number }).value = thickness;
    (u.uScale as { value: number }).value = scale;
    (u.uPixelSize as { value: number }).value = pixelSize;
    (u.uGlow as { value: number }).value = glow;
    (u.uColorMode as { value: number }).value = colorModeToFloat(colorMode);
    (u.uContrast as { value: number }).value = contrast;
    (u.uBrightness as { value: number }).value = brightness;
    (u.uFillBands as { value: number }).value = fillBands ? 1.0 : 0.0;
    (u.uOpacity as { value: number }).value = opacity;
    (u.uLightMode as { value: number }).value = lightMode ? 1.0 : 0.0;
    (u.uGrain as { value: number }).value = grain ? 1.0 : 0.0;
    (u.uGrainIntensity as { value: number }).value = grainIntensity;
    (u.uLow as { value: Float32Array }).value = new Float32Array(hexToRgb(lowColor));
    (u.uMid as { value: Float32Array }).value = new Float32Array(hexToRgb(midColor));
    (u.uHigh as { value: Float32Array }).value = new Float32Array(hexToRgb(highColor));
    (u.uMouseEnabled as { value: number }).value = mouseInteraction ? 1.0 : 0.0;
    (u.uMouseRadius as { value: number }).value = mouseRadius;
