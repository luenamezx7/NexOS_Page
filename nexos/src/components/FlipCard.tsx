...
const FlipCard: React.FC<FlipCardProps> = ({
  ...
  const sumX = useTransform([turn, tiltX], ([t, x]: number[]) => (t as number) + (x as number));
  const sumY = useTransform([turn, tiltY], ([t, y]: number[]) => (t as number) + (y as number));
  ...
  const settle = (to: number, velocity: number, instant: boolean) => {
    ...
    else spin.current = animate(turn, to, { type: 'spring', stiffness, damping, velocity, restDelta: 0.05 } as never);
  ...
  useEffect(() => {
    ...
    else spin.current = animate(turn, target.current, { type: 'spring', stiffness, damping, restDelta: 0.05 } as never);
  ...
  return (
    <div
      ...
      onPointerEnter={(e) => { if (!reduce && !disabled && e.pointerType !== 'touch') lift.set(hoverScale); }}
      onPointerLeave={() => { if (!grip.current) rest(); }}
      onKeyDown={onKeyDown}
      onClick={onClick}
    ...
    >
      {shadow ? (
        <motion.span className="flip-card__shadow" aria-hidden="true" style={axis === 'x' ? { scaleY: spread, opacity: shade } : { scaleX: spread, opacity: shade }} />
      ) : null}
      <motion.div className="flip-card__rotor" style={reduce ? undefined : rotorStyle}>
        <div className="flip-card__face flip-card__face--front" aria-hidden={shown} inert={shown ? true : undefined}>
          {front}
          {glare ? <span className="flip-card__glare" aria-hidden="true" /> : null}
        </div>
        <div className="flip-card__face flip-card__face--back" aria-hidden={!shown} inert={!shown ? true : undefined}>
          {back}
          {glare ? <span className="flip-card__glare" aria-hidden="true" /> : null}
        </div>
      </motion.div>
