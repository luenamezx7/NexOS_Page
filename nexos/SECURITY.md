...
3. **Definir `SITE_URL` e `NEXT_PUBLIC_SITE_URL`** com a origem HTTPS real. Em **Authentication > URL Configuration**, permitir exatamente `https://SEU-DOMINIO/auth/callback` (e localhost apenas para dev). O fluxo usa `{{ .ConfirmationURL }}` PKCE; confirmar no mesmo navegador. O template de "recuperar senha" (Reset Password) deve apontar para o mesmo `{{ .ConfirmationURL }}` — o callback troca o code e leva a `/portal/redefinir` para definir a nova senha.
